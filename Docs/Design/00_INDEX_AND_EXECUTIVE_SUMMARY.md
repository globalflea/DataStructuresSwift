...
flowchart TD
    subgraph Foundation ["MeridianCore Core Subsystems"]
        MC["MeridianCore (DataStructures)<br/>• RingBuffer, CircularEventBuffer<br/>• BTree, PriorityQueue, MonotonicDeque<br/>• ConcurrentMap, ConcurrentSet, Glob<br/>• Segmented WAL, CRC-64, Replication"]
...
| :--- | :--- | :--- | :---: | :---: |
| **`MeridianCore`** | Algorithmic collections, binary storage, replication | `BTree`, `RingBuffer`, `CircularEventBuffer`, `PriorityQueue`, `ConcurrentMap`, `ConcurrentSet`, `SegmentedWALWriter`, `SegmentedWALReader`, `CRC64`, `Glob`, `ReplicationBroadcaster`, `ReplicationFollowerEngine` | **97.10%** | 58 / 58 PASS |
